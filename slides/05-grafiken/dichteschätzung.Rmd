...
```{r, 2-problem-hist, echo = FALSE, fig.height=3.2, fig.width=6, out.height = ".5\\textheight"}
...
densplot + 
  geom_histogram(aes(y = after_stat(density)), bins=10, fill=rgb(1,1,1,.8), col="black", linewidth = .1) +
...
densplot + 
  geom_histogram(aes(y = after_stat(density)), bins=150, fill=rgb(1,1,1,.8), col="black", linewidth = .1) +
  geom_density(col="blue")  +
  ylim(c(0, 0.65))
```
...
```{r, 2-kernelexamples, echo = FALSE, fig.height = 4, fig.width = 6, out.width = ".9\\textwidth", out.height = ".6\\textwidth"}
par(mar = c(4,4,1,0.1))
...
- Histogramme berücksichtigen nicht, *wo* Beobachtungen in der Klasse liegen, zählen nur die *Anzahl der Beobachtungen* innerhalb der Klassengrenzen
- Kerndichteschätzung an der Stelle $x$ berücksichtigt *Entfernung der benachbarten Punkte* zu $x$, mit über die Distanz *abnehmender Gewichtung*:

```{r, 2-kdehist1, echo = FALSE, fig.height=3.2, fig.width=6, out.height = ".5\\textheight"}
par(mar = c(2, 4, 1, 0.1), mfrow = c(1,2))
x = c(0.25, 1.75, 2.95, 3.25, 4.5)
...
k = kde(x, main = "Kerndichte (Gauss-Kern)", col = col, ylim = c(0, 0.4))
...
```{r, 2-kde-bw1, echo = FALSE}
#, fig.height=4, fig.width=9, out.width="2.25\\textheight", out.height="\\textheight"}
...
ggplot(data_kde, aes(x = x)) +  
  ...
  geom_density(col = "purple", bw = 1.2, lwd = 1) 
```

Kerndichteschätzer mit "optimaler" Bandbreite $h$ (blau), zu kleiner (orange) und zu 
großer (lila) Bandbreite $h$.

## Verteilungen grafisch vergleichen

### Schritt für Schritt: 1. Histogramm

Reale Daten (Mietspiegel): Verteilung der Quadratmetermiete, aufbereitet für den
Gruppenvergleich nach *Baujahr-Klasse*.

```{r, 05-build-dist1, echo = TRUE, message = FALSE, fig.height = 3, fig.width = 6, out.height = ".45\\textheight"}
mietspiegel <- mietspiegel |>
  mutate(Baujahr = cut(bj, c(1918, 1945, 1960, 1980, 2013),
    dig.lab = 4, include.lowest = TRUE))

ggplot(mietspiegel, aes(x = nmqm)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30)
```

### Schritt für Schritt: 2. Kerndichte

- `geom_density()` legt eine glatte Kerndichteschätzung über das Histogramm
- unabhängig von Klassengrenzen, gut für (quasi-)stetige Merkmale

```{r, 05-build-dist2, echo = TRUE, message = FALSE, fig.height = 3, fig.width = 6, out.height = ".45\\textheight"}
ggplot(mietspiegel, aes(x = nmqm)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30) +
  geom_density(colour = "blue", linewidth = 1)
```

### Schritt für Schritt: 3. Dichten je Gruppe

- eine Dichte *pro Baujahr-Klasse* (Ästhetik **Farbe** = Gruppe)
- zeigt, wie sich die *bedingten* Verteilungen unterscheiden -- neuere Bauten teurer/$m^2$

```{r, 05-build-dist3, echo = TRUE, message = FALSE, fig.height = 3, fig.width = 6.5, out.height = ".45\\textheight"}
ggplot(mietspiegel, aes(x = nmqm, colour = Baujahr)) +
  geom_density(linewidth = 1)
```

### Schritt für Schritt: 4. Boxplot & Violinplot

- kompakter Gruppenvergleich: *Violinplot* (gespiegelte Dichte) + *Boxplot* innen
- eine Ebene pro `geom_*`, aufeinander gestapelt

```{r, 05-build-dist4, echo = TRUE, message = FALSE, fig.height = 3, fig.width = 6.5, out.height = ".45\\textheight"}
ggplot(mietspiegel, aes(x = Baujahr, y = nmqm)) +
  geom_violin(aes(fill = Baujahr), show.legend = FALSE) +
...
- Abhängigkeit von der Bandbreite $h$  
  $\rightarrow$ Benutze automatische Verfahren zur Bestimmung von "optimalem" $h$ aus den Daten
