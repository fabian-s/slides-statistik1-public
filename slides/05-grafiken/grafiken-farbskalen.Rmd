...
### HCL-Farbraum: drei Dimensionen

- **Farbton** (*hue*): dominante Wellenlänge -- *kreisförmige* Dimension
  (rot - gelb - grün - blau - lila - rot)
- **Farbsättigung** (*chroma*): Intensität / "Buntheit"
- **Helligkeit** (*luminance*): hell -- dunkel

```{r, 05-hcl-wheel, echo = FALSE, out.height = ".3\\textheight", out.width = ".3\\textheight", fig.width = 6}
wheel(hcl(h = seq(0, 360, l = 101), c = 100, l = 60))
```
```{r, 05-hcl-ramps, echo = FALSE, out.height = ".28\\textheight"}
(pal(hcl(h = 150, c = seq(0, 100, l = 61), l = 70))) /
(pal(hcl(h = 100, l = seq(0, 100, l = 101), c = 100)))
```
\footnotesize
oben: variierende Sättigung (konstanter Farbton & Helligkeit);
unten: variierende Helligkeit

### HCL-Farbraum: Zusammenspiel

Farbton (kreisförmig) $\times$ Sättigung (Spalten) $\times$ Helligkeit (Zeilen):

```{r, 05-hcl-grid, echo = FALSE, out.height = ".65\\textheight", out.width = ".65\\textheight", fig.width = 6}
plist <- list()
for (c in (3:1)/3 * 100)
  for (l in (1:3)/3 * 100) {
...
### Beispiel: Qualitative Farbskalen
...
### Beispiel: Sequentielle Farbskalen
...
ev_white <- ggplot(data = us_states_elec,
  ...
  geom_polygon(fill = NA, color = scales::alpha("black", .3), linewidth = 0.1) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  theme_map() +
  guides(fill =
...
### Beispiel: Divergierende Farbskalen (Presse)
...
<!-- OBACHT: Skala ist hier nicht optimal gewählt - nicht gleiche Abstände/Scale nach unten+oben! -->
