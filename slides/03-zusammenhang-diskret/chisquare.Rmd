...
### Beispiel: Nachwahlbefragung 2021 -- Abweichungen

Beobachtete Häufigkeiten $h_{ij}$, eingefärbt nach der Abweichung von der
Unabhängigkeitserwartung $\tilde h_{ij}$:

\vskip 1.5em

\begin{center}
```{r 03-tab-wahl-resid, echo=FALSE, results='asis'}
# gt_contingency() + wahl_abs kommen aus kontingenztafeln.Rmd (Chunk
# 03-gt-contingency-setup) -- Kapitel-Kinder teilen die knit-Umgebung,
# Reihenfolge in main.Rmd beachten; standalone rendert diese Datei nicht.
cat(gt_contingency(wahl_abs, proportional_size = TRUE, size_range = c(7, 16),
...
\textcolor[rgb]{0.843,0.188,0.153}{rot} $=$ seltener, \textcolor[rgb]{0.271,0.459,0.706}{blau} $=$ häufiger als erwartet.
Die Schriftgröße kodiert zusätzlich die Zellhäufigkeit (auch im Graustufendruck lesbar).
