...
### Odds: Zusammenhang mit Kapitel 2

In Kap. 2 war die **Chance** eines Ereignisses $A$ definiert als
$\gamma(A) = \frac{P(A)}{1-P(A)}$ (Wahrscheinlichkeits-Odds).

::: {.block}
#### Empirische Odds $=$ geschätzte Wahrscheinlichkeits-Odds der bedingten Verteilung

Für dichotomes $Y \in \{1, 2\}$ und die Subpopulation $X = a_i$ gilt mit
$p := f_{Y|X}(1|a_i) = \frac{h_{i1}}{h_{i\cdot}}$ (bedingte relative Häufigkeit):
$$
...
Das **Häufigkeitsverhältnis** $\tfrac{h_{i1}}{h_{i2}}$ aus dieser Tabelle ist also
genau die Chance $\gamma(A)$ von Kap. 2, angewandt auf das Ereignis $A = \{Y=1\}$
mit der *bedingten empirischen* Verteilung in der Subpopulation $X = a_i$.
...
-   Verallgemeinerung des Verfahrens auf mehr als zwei Ausprägungen mindestens
