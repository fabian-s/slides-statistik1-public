...
### Bedingte relative Häufigkeit: Berechnung
...
\small
\begin{center}
```{r 03-tab-wahl-abs-2, echo=FALSE, results='asis'}
...
### Anteile vergleichen: Prozentpunkte vs. relative Änderung

Bedingte relative Häufigkeiten (Anteile) werden oft als **Prozente** angegeben --
beim Vergleich zweier Anteile lauern Fallstricke. Beispiel: SPD-Anteil bei
Frauen $27\%$, bei Männern $25\%$.

>- **Prozentpunkte** ($\%$-Punkte): *absolute* Differenz zweier Anteile.
>   $27\% - 25\% = 2$ Prozentpunkte.
>- **relative Änderung**: Anteil der Differenz *am Ausgangswert*.
>   $\frac{27\% - 25\%}{25\%} = 8\%$ -- der Anteil ist um $8\%$ **größer**, aber nur um $2$ Prozentpunkte.
>- **"$x$-mal so groß"** ist eindeutig ($27\% = 1{,}08 \cdot 25\%$).
>   **"$x$-mal größer"** ist strittig (Faktor $x$ oder Zunahme *um* das $x$-fache?)
>   $\longrightarrow$ vermeiden.

### Anteile vergleichen: Asymmetrie

Der Vergleich zweier Anteile ist **nicht symmetrisch** -- Bezugsgröße beachten!

Beispiel: Anteil $A = 110$, Anteil $B = 100$ (in geeigneten Einheiten).

>- $A$ ist um $\frac{110-100}{100} = 10\%$ **größer** als $B$.
>- Aber $B$ ist **nicht** um $10\%$ kleiner als $A$, sondern nur um
>   $\frac{110-100}{110} \approx 9{,}1\%$ (Bezug jetzt auf $110$).
>- $\implies$ Relative Änderungen sind richtungsabhängig; immer angeben,
>   **worauf** sich der Prozentwert bezieht.
