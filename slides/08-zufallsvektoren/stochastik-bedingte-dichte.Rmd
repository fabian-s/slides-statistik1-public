...
### Bedingte Verteilungen stetiger ZV: Problem
...
### Bedingte Verteilungen stetiger ZV: Herleitung
...
Bei stetigen ZV ist $P(Y = y) = 0$, der diskrete Quotient $\tfrac{P(X\le x, Y=y)}{P(Y=y)}$ also nicht definiert. **Idee:** bedinge auf ein winziges Intervall um $y$ und lasse dessen Länge gegen $0$ gehen -- dabei kürzt sich diese Länge heraus und man erhält dieselbe einfache Form wie im diskreten Fall.
...
```{r bedingt-randdichte-plot, fig.height = 3.4, echo = FALSE}
# source("setup.R")
grid <- seq(0, 1, l = 100)
grid2 <- seq(0, 1, l = 20)
...
### Beispiel: Rand- und bedingte Dichte -- Lösung

Gemeinsame Dichte $f_{X,Y}(x, y) = (1 + x - y - 2 x^2 + 4x^2 y)$ auf $[0,1]^2$:

- **Rand-Dichte von $X$?**
- **Bedingte Dichte von $Y|X = x$?**

>- Rand-Dichte: $f_X(x) = \int_0^1 f_{X,Y}(x,y)\,dy = x + \tfrac12$ für $x \in [0,1]$.
>- Bedingte Dichte: $f_{Y|X}(y|x) = \dfrac{f_{X,Y}(x,y)}{f_X(x)} = 2 - 2x + (4x-2)\,y$ für $y \in [0,1]$.
>- $f_{Y|X}(y|x)$ hängt von $x$ ab $\leadsto$ $X$ und $Y$ sind **nicht** unabhängig.
...
### Wichtiges Beispiel: Bayes-Lernen

Dieses Setup ($X \sim {\mathcal B}e(\alpha, \beta)$, $Y|X \sim {\mathcal B}(n, X)$) ist das **Grundmodell des Bayes-Lernens**:

- **gemeinsame** Verteilung, **bedingte** Verteilung $X|Y=y \sim {\mathcal B}e(\alpha + y,\, \beta + n - y)$ (Update der Erfolgswahrscheinlichkeit, nachdem man $y$ Erfolge beobachtet hat) und **Randverteilung** von $Y$ (die *Beta-Binomialverteilung*)


### Bayes-Lernen: Die gemeinsame Verteilung
...
### Bayes-Lernen: Die bedingte Verteilung von $X|Y=y$
...
### Bayes-Lernen: Die Randverteilung von $Y$
