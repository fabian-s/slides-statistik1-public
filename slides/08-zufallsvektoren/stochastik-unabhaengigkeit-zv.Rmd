...
- Äquivalent über **bedingte Dichten** (Kap. "Bedingte Verteilungen"): $X$ und $Y$ sind genau dann unabhängig, wenn $f_{X|Y}(x|y) = f_X(x)$ für alle $x, y$ gilt -- die Bedingung auf $Y=y$ ändert die Verteilung von $X$ dann nicht. (So wird Unabhängigkeit in Blatt 12 nachgewiesen.)
...
```{r, sto-marginal-joint-1, echo = FALSE, out.height = ".62\\textheight"}
...
**Takeaway:** Dichte auf der Diagonalen konzentriert -- aus den Rändern **nicht** rekonstruierbar: $f(x_1, x_2) \neq f_{X_1}(x_1)\, f_{X_2}(x_2)$.

### Beispiel: Randverteilung & Gemeinsame Verteilung

```{r, sto-marginal-joint-2, echo = FALSE, out.height = ".62\\textheight"}
...
**Takeaway:** *Gleiche* Ränder wie links, aber hier gilt $f(x_1, x_2) = f_{X_1}(x_1)\, f_{X_2}(x_2)$ -- bei Unabhängigkeit bestimmen die Ränder die gemeinsame Verteilung vollständig.
